...
const { setBroadcasters, setGsmBroadcaster } = require('./mqtt/client')
...
async function wsPlugin(app) {
  ...
  // ── GSM broadcaster — уведомить админов группы ────────────
  setGsmBroadcaster(async (groupId, data) => {
    try {
      const db     = getDb()
      const admins = await db`
        SELECT ug.user_id FROM user_groups ug
        WHERE ug.group_id = ${groupId}
          AND ug.role IN ('admin', 'superadmin')
      `
      const msg = JSON.stringify({ type: 'gsm_event', ...data })
      for (const { user_id } of admins) {
        sendToUser(user_id, msg)
      }
    } catch {}
